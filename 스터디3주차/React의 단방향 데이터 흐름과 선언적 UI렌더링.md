...
🧐 React를 어떻게 하면데이터 단방향, 상태에 따른 UI 선언적 렌더링하는 React답게 사용할 수 있을까?
...
동일한 입력에 대해 통일한 출력을 반환하는 순수한 컴포넌트를 사용한다.
...
함수형 컴포넌트에 리액트에서 제공하는 hook을 사용하면 함수형 컴포넌트에서 상태 관리, 라이플사이클 메서드를 사용할 수 있다.
...
사용자 상호작용을 통해 상태를 업데이트하려면 이벤트 핸들러를 사용해야 한다.
